...
export const useContentOperations = ({parentType = "section"} = {}) => {
    ...
    // Publish all contents for a given owner (section, module, etc.)
    const publishAllContents = useCallback(
        async (ownerId) => {
            try {
                const response = await axios.post(`/api/content/owner/${ownerId}/publish`);
                return response.data;
            } catch (error) {
