    ...
    <div className="navigation">
      ...
      >
        <ul className="block outline-none cursor-pointer relative w-12 h-12">
          ...
          />
          <li
            className={`absolute left-3 w-6 h-0.5 bg-gray-600 bg-opacity-70 rounded-full overflow-hidden transition-all duration-500
            top-8.5
            ${toggle ? "transform -translate-y-2 -rotate-45" : ""}
          `}
          />
      ...
      >
        <ul
          className={`flex m-0 p-0 text-center list-none w-full ${toggle ? "flex-col justify-center" : "justify-center"} md:flex-row md:justify-center`}
        >
        ...
        `}
              ...
              >
                <span className="overflow-hidden">
                  <div className="font-serif hover:text-white transition-colors duration-300 text-lg md:text-2xl uppercase">
                    {page.name}
                  </div>
                </span>
              </Link>
              ...
              {page.children.length > 0 && (
                <ul
                  className={`
          absolute left-1/2 top-full transform -translate-x-1/2 min-w-[180px] z-50
          bg-blue-600 rounded-b-lg shadow-lg
          flex flex-col
                ...
                >
                  {page.children
                    ...
                    .map((child) => (
                      <li
                        key={child.id}
                        className="px-4 py-2 text-white hover:bg-blue-700 transition-colors duration-200"
                      >
                        <Link
                          to={child.slug}
                          onClick={() => setToggle(false)}
                          className="block w-full text-sm md:text-base uppercase"
                        >
                          <span className="overflow-hidden">
                            <div>{child.name}</div>
                          </span>
