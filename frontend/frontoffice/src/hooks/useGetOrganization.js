...
const useGetOrganization = () => {
    ...
    const fetchOrganizationSettings = async () => {
        ...
        setError(null);
        try {
            const response = await axiosClient.get('/api/public/organization/settings');
            setSettings(response.data);
            return response.data;
        } catch (err) {
            setError(err.response?.data?.message || err.message || 'Error loading organization settings');
            return null;
        } finally {
            setLoading(false);
        }
    ...
    const fetchOrganizationInfo = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.message || err.message || 'Error loading organization information');
            return null;
        } finally {
            setLoading(false);
        }
    ...
    const fetchAllOrganizationData = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.message || err.message || 'Error loading organization data');
            return null;
        } finally {
            setLoading(false);
        }
