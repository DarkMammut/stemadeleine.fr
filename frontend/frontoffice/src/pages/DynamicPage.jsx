...
const DynamicPage = () => {
    ...
    useEffect(() => {
        const loadPage = async () => {
            // Reconstruit le slug complet à partir de l'URL
            const fullSlug = location.pathname.startsWith('/')
                ? location.pathname.substring(1)
                : location.pathname;

            console.log('Chargement de la page avec le slug:', fullSlug);
            ...
            <main className="container mx-auto px-4 py-8">
                ...
                <nav className="mb-8">
                    <ol className="flex items-center space-x-2 text-sm text-gray-500">
                        <li>
                            <a href="/frontend/frontoffice/public" className="hover:text-primary">Accueil</a>
                        </li>
                            ...
                            return (
                                <li key={index} className="flex items-center">
                                    ...
                                    {isLast ? (
                                        ...
                                        <a href={href} className="hover:text-primary capitalize">
                                            {segment.replace('-', ' ')}
                                        </a>
                                    )}
                                </li>
                ...
                {(page.createdAt || page.updatedAt) && (
                    <footer className="mt-12 pt-8 border-t border-gray-200">
                        <div className="text-sm text-gray-500">
                            {page.createdAt && (
                                <p>Créé le : {new Date(page.createdAt).toLocaleDateString('fr-FR')}</p>
                            )}
                            {page.updatedAt && page.updatedAt !== page.createdAt && (
                                <p>Dernière mise à jour : {new Date(page.updatedAt).toLocaleDateString('fr-FR')}</p>
                            )}
                        </div>
                    </footer>
                )}
