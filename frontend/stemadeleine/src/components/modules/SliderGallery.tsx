...
const SliderGallery: React.FC<Props> = ({ module, gallery, loading = false }) => {
    ...
    return (
      <div className="w-full py-6 border-b border-gray-200 last:border-b-0">
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <p className="text-green-600 text-sm">Module Galerie (Slider) - Implémentation à venir</p>
        </div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {gallery.medias.map((m: any) => (
          <div key={m.id} className="rounded-md overflow-hidden bg-gray-100">
            <img src={m.thumbnailUrl ?? m.url} alt={m.alt ?? ''} className="w-full h-56 object-cover" />
          </div>
        ))}
      </div>
    </div>
